<%@page import="com.rental.domain.CommentVO"%>
<%@page import="com.jayway.jsonpath.JsonPath"%>
<%@page import="com.jayway.jsonpath.DocumentContext"%>
<%@include file="../header.jsp"%>
<%@taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c"%>
<c:set var="uuri"><%=request.getAttribute("javax.servlet.forward.request_uri")%></c:set>
${uri = uuri.substring(0,uuri.lastIndexOf("/"));''}
...
<link type="text/css" rel="stylesheet"
	...
	media="screen">
<link rel="stylesheet"
	href="/resources/tourist/css/magnific-popup.min.css" type="text/css"
	media="screen">
<link rel="stylesheet" href="/resources/tourist/css/prism.min.css"
	type="text/css" media="screen">
<link rel="stylesheet" href="/resources/tourist/css/fotorama.css"
	type="text/css" media="screen">
<link rel="stylesheet" href="/resources/tourist/css/venobox.css"
	type="text/css" media="screen">
<link type="text/css" rel="stylesheet"
...
var sid = '<c:out value="${uuri}" />'.split('/')[2]
					...
					<div class="tab-scroll-page-category">
						...
						<div id="tab-page-category">
							<div class="page-category-list">
								...
								<span style='font-size: 24px;'>관광명소</span> <span
									class="fake-span"></span>
							</div>
							<div class="controls">
...
.board-view .board-view-info .view-member-progress {
	position: absolute;
	top: 0;
	right: 0;
	width: 150px
...
.board-view .board-view-info .board-view-info-label {
	position: absolute;
	top: 10px;
	right: 0;
	z-index: 2
...
.board-view .board-view-info .board-view-good-nogood {
	position: relative;
	height: 30px
}
...
.board-view .board-view-info .board-view-nogood {
	display: inline-block;
	width: 100px;
	height: 30px;
	line-height: 30px;
...
.board-view .board-view-info .board-view-good:hover {
	background: #7391F5
}

.board-view .board-view-info .board-view-nogood:hover {
	background: #959595
}

.board-view .board-view-info .board-view-good-nogood .no-member-gng {
	cursor: pointer
}
...
.board-view .board-view-star .collapse-rating-result-panel span.active a
	{
	color: #FF4848
}
...
.board-view .eyoom-form .rating label .fa {
	font-size: 12px
}

.board-view .eyoom-form .rating label .fas {
	font-size: 11px
}
...
.board-view .rating-mb-photo, .board-view .good-mb-photo {
	display: inline-block;
	width: 26px;
	height: 26px;
	margin-right: 2px;
...
.board-view .rating-mb-photo img, .board-view .good-mb-photo img {
	width: 100%;
	height: auto;
	-webkit-border-radius: 50% !important;
	-moz-border-radius: 50% !important;
...
.board-view .rating-mb-photo .rating-user-icon, .board-view .good-mb-photo .good-user-icon
	{
	width: 22px;
	height: 22px;
	font-size: 12px;
...
.board-view .board-view-good-view {
	padding: 7px 0;
...
.board-view .board-view-good-member h6 {
	position: relative;
	text-align: center;
	font-size: 12px;
	margin: 0 0 16px
...
.board-view .board-view-good-member h6:after {
	content: "";
	display: block;
	position: absolute;
	bottom: -8px;
...
.board-view .board-view-short-url {
	font-size: 11px
}

.board-view .board-view-short-url ul {
	margin-bottom: 0
}

.board-view .board-view-short-url li {
	padding: 5px 0;
	border-bottom: 1px dotted #e5e5e5
}

.board-view .board-view-short-url a:hover {
	text-decoration: underline
}
...
.board-view .board-view-btn.board-pin-btn i {
	color: #fff;
	margin-right: 5px
}

.board-view .board-view-btn.board-pin-btn:hover {
	background: #2B2B2E !important
}

.board-view .board-view-btn.board-pin-btn:hover i {
	color: #86A0F7
}
...
.board-view .view-area-divider .divider-circle {
	position: absolute;
	top: -7px;
	left: 50%;
	margin-left: -7px;
...
.board-view .view-dmap-container {
	position: relative;
	min-height: 400px;
	border-width: 1px;
	border: 1px solid #c5c5c5;
...
.board-view .dmap-view-canvas-wrap {
	position: relative;
	height: 390px;
	margin: 0px
}

.board-view #dmap_view_canvas {
	width: 100%;
	height: 100%
}

.board-view .view-dmap-container .dmap-overlay-wrap {
	position: absolute;
	bottom: 35px;
	left: -150px;
	width: 300px
...
.board-view .view-dmap-container .dmap-overlay {
	position: relative;
	width: 100%;
	border: 0;
	background: #3C3C3E
...
.board-view .view-dmap-container .dmap-overlay-wrap .after {
	content: "";
	position: relative;
	margin-left: -10px;
	left: 50%;
...
.board-view .view-dmap-container .dmap-overlay .dmap-overlay-title {
	position: relative;
	overflow: hidden;
	display: block;
	height: 40px;
...
.board-view .view-dmap-container .dmap-overlay .dmap-overlay-title:hover
	{
	background: #4069EE;
...
.board-view .view-dmap-container .dmap-overlay-desc {
	padding: 10px
}

.board-view .view-dmap-container .dmap-overlay-desc p {
	position: relative;
	font-size: 11px;
	color: #d5d5d5;
	padding-left: 12px;
...
.board-view .view-dmap-container .dmap-overlay-desc p:before {
	content: "";
	position: absolute;
	top: 6px;
	left: 0;
...
.board-view .view-dmap-container .dmap-overlay-close {
	position: absolute;
	top: 40px;
	right: 0px;
	color: #fff;
...
.board-view .view-dmap-container .dmap-overlay-close:hover {
	background: #2B2B2E;
	-webkit-transition: all 0.2s ease-in-out;
	-moz-transition: all 0.2s ease-in-out;
	-o-transition: all 0.2s ease-in-out;
...
@media ( max-width :767px) {
	...
	.board-view .dmap-view-info-wrap {
		width: auto;
		float: none
	}
}
	...
	font-size: 24px;
					...
					<article class="board-view">
						<h4 style='font-size: 22px;'>
							<span class="color-grey margin-right-5">[<%=getRegion((String) (pageContext.getAttribute("region2")))%>]
							</span> <strong class='title'>${tourInfo.title}</strong>
						</h4>
						<div class="board-view-info">
							<div class="view-post-info">
								<div class="view-photo-box">
									<span class="view-photo margin-right-5"><img
										class="user-photo" src="/resources/tourist/image/admin.png"
										alt="회원사진"></span>
								</div>
								<div class="view-info-box">
									<div class="info-box-top">
										<span class="view-nick"> <span class="sv_wrap">
												<a href="http://theme4.eyoom.net/bbs/profile.php?mb_id=ffff"
												data-toggle="dropdown" title="할인의추억 자기소개" target="_blank"
												rel="nofollow" onclick="return false;"> 최고관리자 </a>
									...
									<div class="info-box-bottom">
										...
										<span><i class="fas fa-eye"></i>조회수 ${tourInfo.views}</span> <span
											class="color-red"><i class="fas fa-comment-alt"></i>${tourInfo.comments_num }</span>
										<!--                     <div style='postion:absolute'> -->
										<sec:authorize access="isAnonymous()">
											<span class="color-grey font-size-11 margin-left-5"
												style='float: right;'><i
												class="fas fa-exclamation-circle"></i> 로그인 후 좋아요 / 즐겨찾기 가능</span>
										</sec:authorize>
										<!--     </div>    -->
									</div>
								</div>
	...
	padding-top: 8px;
										...
										<div id='heart_container' style='margin-bottom: 30px;'>
											<div class="board-view-good no-member-gng" title="좋\n아\n요">
												<div
													id='<sec:authorize access="isAuthenticated()">handle</sec:authorize>Like'>
													<i class="fa${tourInfo.like_true ? '': 'r'} fa-heart"
														style="color: red"></i> <strong class='heart_click'>${tourInfo.likes}</strong>
												</div>
											</div>
											<div class="board-view-good no-member-gng" title="즐겨찾기">
												<!-- #ffd700 -->
												<div
													id='favor<sec:authorize access="isAuthenticated()" >Star</sec:authorize>'>
													<i class="fa${tourInfo.favor_true ? '':'r'} fa-star"
														style='color:${tourInfo.favor_true ? "yellow":"white"}'></i>
       				...
       				console.log('hello else')
						...
						<div class="view-top-btn">
							<ul class="top-btn-right list-unstyled pull-right">
								...
								<li><a class="btn-e btn-e-dark" type="button"
									data-toggle="modal" data-target=".search-modal"> <i
										class="fas fa-search"></i>
								</a></li>
								<!-- 검색기능 -->
								<li><a href="/tourist" class="btn-e btn-e-dark"
									type="button">목록</a></li>
							</ul>
						...
						<div class="board-view-atc">
							...
							<div class="board-view-file-conts">
								<div class="fotorama" data-nav="thumbs" data-max-width="100%"
									data-loop="true" data-allowfullscreen="true"
									data-thumbwidth="170" data-thumbheight="100" data-width="1200">
									<img
										src="<%=getPhoto((String) (pageContext.getAttribute("photo")))%>"
										data-full="<%=getPhoto((String) (pageContext.getAttribute("photo")))%>"
										data-caption=""> <img
										src="http://theme4.eyoom.net/data/file/dmap/1218034632_iVo9nuOe_41d8867af7c0b5e79c782231869409f2cd9f3569.jpg"
										data-full="http://theme4.eyoom.net/data/file/dmap/1218034632_iVo9nuOe_41d8867af7c0b5e79c782231869409f2cd9f3569.jpg"
										data-caption=""> <img
										src="http://theme4.eyoom.net/data/file/dmap/1218034632_Vdngq5pW_d48ed61e805f75cdbb3cef73b9e80bd20418fa39.jpg"
										data-full="http://theme4.eyoom.net/data/file/dmap/1218034632_Vdngq5pW_d48ed61e805f75cdbb3cef73b9e80bd20418fa39.jpg"
										data-caption=""> <img
										src="http://theme4.eyoom.net/data/file/dmap/1218034632_1pZ4ONz9_81360149c270ddef8649104b9f7a891c89dcda6d.jpg"
										data-full="http://theme4.eyoom.net/data/file/dmap/1218034632_1pZ4ONz9_81360149c270ddef8649104b9f7a891c89dcda6d.jpg"
										data-caption=""> <img
										src="http://theme4.eyoom.net/data/file/dmap/1218034632_2eyYsQWg_d87a3db0e7ad22141f7f29576800524324cdc071.jpg"
										data-full="http://theme4.eyoom.net/data/file/dmap/1218034632_2eyYsQWg_d87a3db0e7ad22141f7f29576800524324cdc071.jpg"
										data-caption=""> <img
										src="http://theme4.eyoom.net/data/file/dmap/1218034632_8Fs5nCbU_b3e5a6af46cee9f7263e46529704d759c265dc63.jpg"
										data-full="http://theme4.eyoom.net/data/file/dmap/1218034632_8Fs5nCbU_b3e5a6af46cee9f7263e46529704d759c265dc63.jpg"
										data-caption=""> <img
										src="http://theme4.eyoom.net/data/file/dmap/1218034632_1SAML3ym_d1393648451d1a90b776a2be55717eb0184de19b.jpg"
										data-full="http://theme4.eyoom.net/data/file/dmap/1218034632_1SAML3ym_d1393648451d1a90b776a2be55717eb0184de19b.jpg"
										data-caption=""> <img
										src="http://theme4.eyoom.net/data/file/dmap/1218034632_MNLsnUk0_54edd1fe7f8bc82292a7626b9d887787853361e1.jpg"
										data-full="http://theme4.eyoom.net/data/file/dmap/1218034632_MNLsnUk0_54edd1fe7f8bc82292a7626b9d887787853361e1.jpg"
										data-caption="">
								</div>
							</div>
    ...
    $('.eb-signature-tabs li a').hover(function (e) {
        ...
        $(this).tab('show');
						...
						<ul class="board-view-sns social-icons social-icons-color">
							<li><a
								href="http://theme4.eyoom.net/bbs/sns_send.php?longurl=http%3A%2F%2Ftheme4.eyoom.net%2Fdmap%2F9%3Fsca%3D%25EC%25A0%259C%25EC%25A3%25BC&amp;title=%5B%EC%83%98%ED%94%8C%5D+50%EB%85%84+%EC%A0%84%ED%86%B5%EC%9D%98+%ED%95%B4%EC%82%B0%EB%AC%BC+%EC%A0%84%EB%AC%B8%EC%A0%90%2C+%EC%A0%9C%EC%A3%BC+%EA%B0%90%EC%88%98+%ED%95%B4%EC%82%B0%EB%AC%BC&amp;sns=facebook"
								target="_blank" title="Facebook" class="social_facebook"></a></li>
							<li><a
								href="http://theme4.eyoom.net/bbs/sns_send.php?longurl=http%3A%2F%2Ftheme4.eyoom.net%2Fdmap%2F9%3Fsca%3D%25EC%25A0%259C%25EC%25A3%25BC&amp;title=%5B%EC%83%98%ED%94%8C%5D+50%EB%85%84+%EC%A0%84%ED%86%B5%EC%9D%98+%ED%95%B4%EC%82%B0%EB%AC%BC+%EC%A0%84%EB%AC%B8%EC%A0%90%2C+%EC%A0%9C%EC%A3%BC+%EA%B0%90%EC%88%98+%ED%95%B4%EC%82%B0%EB%AC%BC&amp;sns=twitter"
								target="_blank" title="Twitter" class="social_twitter"></a></li>
							<li><a
								href="http://theme4.eyoom.net/bbs/sns_send.php?longurl=http%3A%2F%2Ftheme4.eyoom.net%2Fdmap%2F9%3Fsca%3D%25EC%25A0%259C%25EC%25A3%25BC&amp;title=%5B%EC%83%98%ED%94%8C%5D+50%EB%85%84+%EC%A0%84%ED%86%B5%EC%9D%98+%ED%95%B4%EC%82%B0%EB%AC%BC+%EC%A0%84%EB%AC%B8%EC%A0%90%2C+%EC%A0%9C%EC%A3%BC+%EA%B0%90%EC%88%98+%ED%95%B4%EC%82%B0%EB%AC%BC&amp;sns=gplus"
								target="_blank" title="Google Plus" class="social_google"></a></li>
							<li><a
								href="http://theme4.eyoom.net/bbs/sns_send.php?longurl=http%3A%2F%2Ftheme4.eyoom.net%2Fdmap%2F9%3Fsca%3D%25EC%25A0%259C%25EC%25A3%25BC&amp;title=%5B%EC%83%98%ED%94%8C%5D+50%EB%85%84+%EC%A0%84%ED%86%B5%EC%9D%98+%ED%95%B4%EC%82%B0%EB%AC%BC+%EC%A0%84%EB%AC%B8%EC%A0%90%2C+%EC%A0%9C%EC%A3%BC+%EA%B0%90%EC%88%98+%ED%95%B4%EC%82%B0%EB%AC%BC&amp;sns=kakaostory"
								target="_blank" title="Kakao Story" class="social_kakaostory"></a></li>
							<li><a
								href="http://theme4.eyoom.net/bbs/sns_send.php?longurl=http%3A%2F%2Ftheme4.eyoom.net%2Fdmap%2F9%3Fsca%3D%25EC%25A0%259C%25EC%25A3%25BC&amp;title=%5B%EC%83%98%ED%94%8C%5D+50%EB%85%84+%EC%A0%84%ED%86%B5%EC%9D%98+%ED%95%B4%EC%82%B0%EB%AC%BC+%EC%A0%84%EB%AC%B8%EC%A0%90%2C+%EC%A0%9C%EC%A3%BC+%EA%B0%90%EC%88%98+%ED%95%B4%EC%82%B0%EB%AC%BC&amp;sns=band"
								target="_blank" title="Band" class="social_band"></a></li>
						</ul>
						<!-- 	태그 -->
						<style>
.icon_tag {
	...
	cursor: pointer;
						...
						<div id='tag_area'>
							<h5>
								<i class="fas fa-tags icon_tag"
									style='line-height: 1.5; cursor: auto'> TAG </i>
								<c:forEach items="${tags}" var="tag">
									<div class='icon_tag' style='font-weight: bold'>
										<a href="${uri}?TagName=${tag}" style='color: white'>#${tag}</a>
									</div>
								</c:forEach>
							</h5>
						</div>
...
.view-comment .view-comment-item .no-comment {
	text-align: center;
	padding: 10px 0 20px
}
...
.view-comment .view-comment-no-item .no-comment {
	color: #959595;
	font-size: 13px
}

.view-comment .view-comment-item .view-cmtgo-btn {
	position: absolute;
	top: -10px;
	left: 0;
	width: 20px;
...
.view-comment .view-comment-item .view-cmtgo-btn:hover {
	background: #555555
}

.view-comment .view-comment-item .view-comment-best-label {
	position: absolute;
	top: -10px;
	left: 20px;
	width: 70px;
...
.view-comment .view-comment-depth {
	position: absolute;
	top: -40px;
	left: 0;
	width: 8px;
...
.view-comment .view-comment-depth i {
	position: absolute;
	top: 116px;
	left: 5px;
	color: #d5d5d5
...
.view-comment .view-comment-photo.no-depth-photo {
	left: 0
}
...
.view-comment .comment-item-info>span {
	margin-right: 5px
}
...
.view-comment .comment-item-content .comment-file-item:after {
	content: "";
	display: block;
	clear: both
}

.view-comment .comment-item-content .comment-file-item span {
	margin-left: 5px
}
...
.view-comment .comment-item-content .comment-cont-wrap {
	display: flex
}

.view-comment .comment-item-content .comment-cont-wrap:after {
	content: "";
	display: block;
	clear: both
}

.view-comment .comment-item-content .comment-cont-img {
	position: relative;
	flex-grow: 1;
	padding-right: 15px
}
...
.view-comment .comment-item-content .comment-yello-card {
	position: relative;
	padding: 10px;
	margin-bottom: 20px;
	background: #ffbeaa;
...
.view-comment .comment-btn-wrap .comment-btn:nth-child(odd) {
	background: #59595B
}

.view-comment .comment-btn-wrap .comment-btn:nth-child(even) {
	background: #676769
}
...
.view-comment .comment-item-body-pn .comment-item-info {
	padding-left: 10px
}
...
.comment-area .comment-write-heading .cmt-point-info-btn {
	position: absolute;
	top: 9px;
	right: 0
}

.comment-area .comment-write-wrap {
	position: relative
}
...
.comment-area .comment-write .comment-write-option .panel-default {
	border: 0;
	margin-bottom: 0;
	box-shadow: none;
...
.comment-area .comment-write .comment-write-option .panel-in {
	position: relative;
	padding: 10px
}

.comment-area .comment-write .comment-write-option .panel-in .panel-in-left
	{
	float: left
}

.comment-area .comment-write .comment-write-option .panel-in .panel-in-right
	{
	float: right
}
...
.comment-area .comment-write .comment-write-option .comment-option-btn:hover
	{
	color: #000;
	background: #e5e5e5
}
...
.comment-area .comment-write .comment-write-option .comment-collapse-box label
	{
	margin-bottom: 0
}

.comment-area .comment-write .comment-write-option .comment-collapse-box .btn-e-input
	{
	padding: 5px 16px
}
...
.comment-area .comment-write-submit {
	position: relative;
	text-align: center
}

.comment-area #modal_comment_video_note .table-list-eb .table thead>tr>th
	{
	text-align: left
}
...
.comment-area .eyoom-form .textarea {
	margin-bottom: 0
}
...
.comment-area-divider .divider-circle {
	position: absolute;
	top: -7px;
	left: 50%;
	margin-left: -7px;
							...
							<div class="view-comment">
								...
								<c:if test="${id.length() < 4}">
								   ...
								   ${id = USER.nickname +='(' +=id.substring(0,id.length())+='*****)';''}
										        ...
										        <div class="view-comment-photo "><img class="user-photo" src="/resources/tourist/image/admin.png" alt="회원사진"></div>
...
<!-- 														<a -->
<!-- 														href="http://theme4.eyoom.net/bbs/profile.php?mb_id=iiii" -->
<!-- 														data-toggle="dropdown" title="백마탄환자 자기소개" target="_blank" -->
<!-- 														rel="nofollow" onclick="return false;"> 백마탄환자</a> -->
<!-- 														<ul class="sv dropdown-menu" role="menu"> -->
<!-- 															<li><a href="http://theme4.eyoom.net/?iiii"><strong>백마탄환자</strong>님의 -->
<!-- 																	홈</a></li> -->
<!-- 															<li><a -->
<!-- 																href="http://theme4.eyoom.net/bbs/new.php?mb_id=iiii">전체게시물</a></li> -->
<!-- 															<li><a -->
<!-- 																href="http://theme4.eyoom.net/blog?sca=&amp;sfl=mb_id,1&amp;stx=iiii">아이디로 -->
<!-- 																	검색</a></li> -->
<!-- 														</ul> -->
...
<!-- 									</div> -->
											...
											<div class="comment-item-body">
													...
													<span class="comment-name"><span class="sv_wrap">
															...
															<ul class="sv dropdown-menu" role="menu">
																<li><a href="http://theme4.eyoom.net/?eeee"><strong>${comment.name}</strong>님의
																		홈</a></li>
																<li><a
																	href="http://theme4.eyoom.net/bbs/new.php?mb_id=eeee">전체게시물</a></li>
																<li><a
																	href="http://theme4.eyoom.net/blog?sca=&amp;sfl=mb_id,1&amp;stx=eeee">아이디로
																		검색</a></li>
															</ul>
												...
												<div class="comment-item-content">
													<div class="comment-cont-wrap">
														...
														<div class="comment-cont-txt">${comment.content}</div>
													</div>
													<div class="margin-bottom-10"></div>
				...
				<div class="comment-area-divider">
					<span class="divider-circle"></span>
				</div>
				<div id="map_canvas"></div>

				<script type="text/javascript"
					src="/resources/tourist/js/venobox.min.js"></script>
				<script type="text/javascript"
					src="/resources/tourist/js/magnific-popup.min.js"></script>
				<script type="text/javascript"
					src="/resources/tourist/js/sweetalert.min.js"></script>
    ...
    $('.comment-image').magnificPopup({
        ...
        image: {
            tError: '이미지를 불러올수 없습니다.'
				...
				<script type="text/javascript"
					src="/resources/tourist/js/fakeLoader.min.js"></script>
				<script type="text/javascript"
					src="/resources/tourist/js/viewimageresize.js"></script>
				<script type="text/javascript"
					src="/resources/tourist/js/sweetalert.min.js"></script>
				<script type="text/javascript"
					src="/resources/tourist/js/clipboard.min.js"></script>
				<script type="text/javascript"
					src="/resources/tourist/js/magnific-popup.min.js"></script>
				<script type="text/javascript"
					src="/resources/tourist/js/fotorama.js"></script>
				<script src="https://developers.kakao.com/sdk/js/kakao.js"></script>
				<script type="text/javascript"
					src="//dapi.kakao.com/v2/maps/sdk.js?appkey=0d721fd94b9d5e3dbe29b561271f021f"></script>
    ...
    $.post(
        ...
        function(data) {
            ...
            if (data.count) {
				...
				<script type="text/javascript"
					src="/resources/tourist/js/vendor_plugins.min.js"></script>
				<script type="text/javascript"
					src="/resources/tourist/js/sly.min.js"></script>
				<script>
$(function() {
	...
	<script>
			...
			//////////////////////////댓글 작성하기
			var commentAdd = function(htmls,sub_comment){
				var id = '<sec:authentication property="name"/>'
				var temp = '<textarea class="form-control" style="border-radius: 5px;"></textarea>'
						if (id== 'anonymousUser'){id = '로그인이 필요합니다' }
						else{
							if (id.length>4){id=id.substring(0,4)+'*'.repeat(4)}
							else{id = id.substring(0,id.length-1)+'*'.repeat(5)}
							id = '${USER.nickname}('+id+')'}
						htmls +='<h2 class="writing">'+id+'님의 <span class="sound_only">댓글의</span> 댓글</h2>'
		            htmls +='<div id="c_10" class="view-comment-item writing" style="margin-left:15px;"><div class="view-comment-depth"><i class="fas fa-caret-right"></i></div>'
		            htmls +='<div class="view-comment-photo "><img class="user-photo" src="/resources/tourist/image/admin.png" alt="회원사진"></div>'
		            htmls +='<div class="comment-item-body"><div class="comment-item-info"><span class="comment-name"><span class="sv_wrap">'
		            htmls +='<a href="http://theme4.eyoom.net/bbs/profile.php?mb_id=ffff" data-toggle="dropdown" title="'+id+' 자기소개" target="_blank" rel="nofollow" onclick="return false;"> '+id+'</a>'
		            htmls +='<ul class="sv dropdown-menu" role="menu"><li><a href="http://theme4.eyoom.net/?ffff"><strong>'+id+'</strong>님의 홈</a></li>'
		            ...
		            htmls +='<div class="comment-cont-txt" style="width:70%;">'+temp+'</div><button class="write" style="background-color: #337ab7;border-radius: 5px;opacity:0.8;color: white;font-weight:bold;margin-left:6px;"> 작성하기 </button></div><div class="margin-bottom-10"></div>'
		            htmls +='<div class="sub_comments" comment_id='+'temp'+' comment_group='+'temp'+'></div></div>'
				sub_comment.append(htmls)
			}
			$(document).on('click','.write',function(){
				...
				var id = '<sec:authentication property="name"/>'
					if (id== 'anonymousUser'){ 
					let True =  confirm('로그인을 해주세요~!')
					if(True){window.location.href ='/login';return;	}
					}
				var comment_id = sub_comment.attr('comment_id')
				var comment_group = sub_comment.attr('comment_group')
				if ((comment_id==0) && (comment_group ==0)){
					var comment_id = null
					var comment_group = null
				}
				var cnt = sub_comment.find('textarea').val()
				var name = '${USER.nickname}'
				$.ajax({
						url : '/tourist/reply' ,
						type: 'post',
						data : {"cid": comment_id,"sid":sid,"cgp":comment_group,'name':name,'content':cnt},
						dataType:'json',
					})
					.done(function(result){
			...
			$(document).on('click','.alert',function(){
				...
				var self = $(this)
				var flag = self.text().trim()
				sub_comment = $(this).closest('.view-comment-item ').find('.sub_comments:first')
				comment_id = sub_comment.attr('comment_id')
				comment_group = sub_comment.attr('comment_group')
				
				if(flag=='댓글'){
					sub_comment.fadeIn(0)
					document.querySelectorAll('.writing').forEach((i)=>{i.remove()})
					var htmls=''
						commentAdd(htmls,sub_comment)
				}
					...
					$.ajax({
						...
						data : {"cid": comment_id,"sid":sid,"cgp":comment_group},
						dataType:'json',
						})
						.done(function(result){
							...
							$.each(result,function(idx,i){
								var a = ''
								var id = i.user_id
								if(i.reply_num>0){a = i.reply_num}
									if (id.length>4){id=id.substring(0,4)+'*'.repeat(4)}
									else{id = id.substring(0,id.length-1)+'*'.repeat(5)}
									id = i.name+'('+id+')'
						        htmls +='<h2>'+id+'님의 <span class="sound_only">댓글의</span> 댓글</h2>'
						            htmls +='<div id="c_10" class="view-comment-item " style="margin-left:15px;"><div class="view-comment-depth"><i class="fas fa-caret-right"></i></div>'
						            htmls +='<div class="view-comment-photo "><img class="user-photo" src="/resources/tourist/image/admin.png" alt="회원사진"></div>'
						            htmls +='<div class="comment-item-body"><div class="comment-item-info"><span class="comment-name"><span class="sv_wrap">'
						            htmls +='<a href="http://theme4.eyoom.net/bbs/profile.php?mb_id=ffff" data-toggle="dropdown" title="'+id+' 자기소개" target="_blank" rel="nofollow" onclick="return false;"> '+id+'</a>'
						            htmls +='<ul class="sv dropdown-menu" role="menu"><li><a href="http://theme4.eyoom.net/?ffff"><strong>'+id+'</strong>님의 홈</a></li>'
...
//        						htmls += ("등록된 댓글이 없습니다.")
						...
						if(sub_comment.css("display") == 'none'){
							document.querySelectorAll('.writing').forEach((i)=>{i.remove()})
							commentAdd('',sub_comment)
						}
						sub_comment.fadeToggle(0);
