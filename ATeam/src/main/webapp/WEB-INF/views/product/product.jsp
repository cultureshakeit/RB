						...
						<c:when test="${pro.status !=0 }">
							<sec:authorize access="isAuthenticated()">
								<button class="genric-btn success-border circle"
									...
									onclick="javascript:location.href='/product/view?num=${pro.num }&userid=${pro.userid }&pageNum=${pageMaker.cri.pageNum }&nickname=<sec:authentication property="principal.member.nickname"/>'">제품 세부 정보</button>
							</sec:authorize>
							<sec:authorize access="isAnonymous()">
								<button class="genric-btn success-border circle"
									style="background: none; color: black;" id="inview"
									data-day="${fn:substring(pro.lastdate,0,10) }"
									onclick="javascript:location.href='/product/view?num=${pro.num }&userid=${pro.userid }&pageNum=${pageMaker.cri.pageNum }'">제품 세부 정보</button>
							</sec:authorize>
						</c:when>
						<c:when test="${pro.status == 0 }">
							<button class="genric-btn success-border circle"
								style="background: none; border-color: red; color: black;"
								onclick='NoRes();'>예약 불가</button>
						</c:when>
					</c:choose>
				</div>
			</c:forEach>
...
<script type="text/javascript">
	function NoRes() {
		alert("재고가 부족합니다. 문의 해주시길 부탁드립니다.");
		return false;
	}
</script>
<script type="text/javascript">
...
<script type="text/javascript">
	...
	for (var i = 0; i < days.length; i++) {
		...
		if (compareToDate(today, daying)) {
			...
			$(days[i]).text("대여 기간 초과");
			$(days[i]).css({
				"border-color" : "red",
				"color" : "red"
			});
			$(days[i]).attr("onclick", "alert('임대 종료 기간이 지났습니다.')")
		}
	}
